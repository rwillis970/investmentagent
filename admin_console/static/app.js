...
const card=(name,value,extra='')=>`<article><span>${esc(name)}</span><strong class="${value==='FAIL'?'fail':''}">${esc(value)}</strong>${extra?`<small>${esc(extra)}</small>`:''}</article>`;
async function act(label,action){await fetch(`/api/services/${encodeURIComponent(label)}/${action}`,{method:'POST'}); await load();}
async function utility(name){const r=await fetch(`/api/utilities/${name}`,{method:'POST'}); document.querySelector('#utility-output').textContent=JSON.stringify(await r.json(),null,2); await load();}
