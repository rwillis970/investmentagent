...
the chain) is followed exactly, unchanged: `_reconcile_accounts` runs before
`audit_log.verify()` is checked below. I considered whether verification
...
except a broken audit chain appends one `mode_transition` event (before ->
whatever was persisted, after -> "PAUSED") and then re-raises the original
exception. PAUSED, not "leave mode untouched": §9.2 makes DISABLED and
PAUSED reachable immediately and unconditionally from any state specifically
so a kill-switch-shaped transition is never blocked by the same state
machine it exists to override, and a failed startup is exactly a case where
the system needs to land somewhere that provably cannot resume into live
...
failed, and continuing to append to it -- even a note about pausing --
...
class AccountReconciliation:
    """One account's worth of input to the reconcile step. `day_trade_guard`
    is the account's own local counter; `broker_reported_day_trades` is
    whatever the broker adapter's account snapshot reports for the same
    window -- see `BrokerAdapter`/`SimulatorBroker.account_data().
    day_trade_count` for where that number comes from in practice."""
    account_id: str
...
def _halt(audit_log: AuditLog, *, persisted_mode: str | None, now: datetime,
         correlation_id: str | None) -> None:
    """Append the one PAUSED note a halting failure leaves behind (DECISION
    2). Never called for a broken chain -- that path raises before this."""
    audit_log.append(actor="system", action="mode_transition",
                     object_type="mode", object_id="system",
                     before=persisted_mode, after="PAUSED",
...
def run_startup(*, target_mode: str, persisted_mode: str | None,
    ...
    except mode_fsm.ModeTransitionError:
        _halt(audit_log, persisted_mode=persisted_mode, now=now, correlation_id=correlation_id)
        raise
    ...
    except market_calendar.CalendarExpiryError:
        _halt(audit_log, persisted_mode=persisted_mode, now=now, correlation_id=correlation_id)
        raise

    # -- §8.1 step 1: reconcile, per account. Per accounts.py there is no
    #    path that combines accounts, so this is a plain loop, and a
    #    mismatch is a halt, not a skip: the loop stops at the first bad
    #    account rather than reconciling the rest and reporting a partial
    #    result.
    reconciled: list[str] = []
    for acct in accounts:
        ...
        except (CrossAccountError, PostureMismatch, market_calendar.CalendarCoverageError):
            _halt(audit_log, persisted_mode=persisted_mode, now=now, correlation_id=correlation_id)
            raise
        audit_log.append(
            actor="system", action="reconcile", object_type="account",
