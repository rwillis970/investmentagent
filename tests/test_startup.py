...
from agent.startup import (AccountReconciliation, AuditChainBroken,
                           StartupResult, run_startup)
...
def account(account_id="acct-a", broker_reported=0, round_trips=()):
    ...
    return AccountReconciliation(account_id=account_id, day_trade_guard=guard,
...
def test_happy_path_reconciles_verifies_sweeps_and_returns_ready_to_resume():
    ...
    assert actions == ["reconcile", "reconcile", "approval_expired", "startup_complete"]
...
def test_illegal_mode_transition_halts_before_any_reconciliation():
    ...
    assert [ev.action for ev in log.events] == ["mode_transition"]
    assert log.events[0].after == "PAUSED"
    assert log.verify() is True
...
def test_confirmation_required_edge_also_halts():
    ...
    assert log.events[-1].after == "PAUSED"
...
def test_cross_account_mismatch_halts_and_does_not_reconcile_remaining_accounts():
    ...
    mismatched = AccountReconciliation(account_id="acct-b", day_trade_guard=guard_b,
    ...
    actions = [ev.action for ev in log.events]
    assert actions == ["reconcile", "mode_transition"]   # acct-a reconciled, acct-c never reached
    assert log.events[-1].after == "PAUSED"


def test_cross_account_error_from_a_misassigned_guard_halts():
    guard = DayTradeGuard(account_id="acct-a", max_per_5_sessions=3)
    wrong = AccountReconciliation(account_id="acct-wrong", day_trade_guard=guard,
...
def test_broken_hash_chain_halts_and_nothing_further_is_written():
    ...
    assert log.events[-1].action == "reconcile"
